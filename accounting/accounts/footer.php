  ...
  <footer id="footer" class="footer">
    <div class="copyright">
      &copy;<strong><span>DACI</span></strong> | 2023
    </div>
    <div class="credits">
  ...
  <?php
      $salechart2 = $mysqli->query("SELECT a.* , b.* , (sum(a.qty)* b.item_price) total ,IF(a.is_walkin= 0, 'Online', 'Walk-In') as wvo from pos_order a 
	  LEFT JOIN pos_items b on b.item_code = a.item_code where a.status = 1 GROUP BY a.is_walkin");

	


	while($valchart2 = $salechart2->fetch_object()){ 
					$chartprice =  $valchart2->qty * $valchart2->item_price;
				    $test11[] = array($valchart2->wvo, $chartprice);
  
	}
	echo json_encode($test11);
  ?>
   <script>
   Highcharts.chart('container-wvo', {
    chart: {
        type: 'column'
    },
    title: {
        text: 'WALK IN VS. ONLINE SALES'
    },
    subtitle: {
    ...
    series: [{
        name: 'Sales',
        data: <?php echo json_encode($test11);?> ,
        dataLabels: {
            enabled: true,
            rotation: -90,
            color: '#FFFFFF',
