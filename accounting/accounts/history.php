    ...
    <div class="pagetitle">
      ...
      <nav>
        <ol class="breadcrumb">
          ...
          <li class="breadcrumb-item">Order Transaction</li>
				...
				?>
                  ...
                  <tr class="order-sel <?php echo $paidBG ?>" onClick="getOrderByID('<?= $val->transcode ?>','<?= $val->status ?>','<?= $val->amount ?>','<?= $val->transcode ?>','<?= $val->customer_id ?>','<?= $val->total_amount ?>','<?= $val->discounted_amount ?>','<?= $val->discount_1 ?>','<?= $val->discount_2 ?>')">
                    ...
                    <td class="text-center"> <?php 
							 if($val->payment_method ==1){ echo "Cash On Delivery"; } 
							 else if($val->payment_method ==2){ echo "GCASH"; } 
							 else if($val->payment_method ==3){ echo "Walk In"; } 
							 ?> </td>
