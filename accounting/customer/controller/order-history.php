...
$orders     = $mysqli->query("SELECT a.* ,b.* from pos_order a left join pos_items b on a.item_code = b.item_code where a.status = 1 and a.customer_id='$customerid' group by a.transcode ");

$checkout   = $mysqli->query("	SELECT a.* ,b.*, c.* , sum(b.item_price * a.qty)price,c.status as order_status from pos_order a 
								left join pos_items b on a.item_code = b.item_code 
